...
let write = (buffer: Luv.Buffer.t, {bytes, idx, totalSize}) => {
	...
	if (bytesToWrite < size) {
		let bufferToWrite = Luv.Buffer.sub(~offset=0, ~length=bytesToWrite, buffer);
		let remainingBuffer = Luv.Buffer.sub(~offset=bytesToWrite, ~length=size - bytesToWrite, buffer);
