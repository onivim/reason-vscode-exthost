...
module Test = {
  ...
  let startWithExtensions = (~handler=noopHandler, ~onError=noopErrorHandler, extensions) => {
    let messages = ref([]);
  ...
  let initData =
    ...
    let client = Client.start(~namedPipe=pipe, ~initData, ~handler=wrappedHandler, ~onError, ()) |> Result.get_ok;
    ...
    let extensionProcess = Node.spawn(
      ~additionalEnv=[
      (
        "AMD_ENTRYPOINT",
  ...
  let waitForProcessClosed = ({processHasExited, _}) =>{ Waiter.wait(
  ~timeout=10.0,
  ~name="Wait for node process to close",
  () => processHasExited^);
  };

  let waitForMessage = (~name, f, {messages, _} as context) => {
    Waiter.wait(
      ~name="Wait for message: " ++ name,
      () => List.exists(f, messages^));
      
      context;
  }
  let waitForReady = (context) => {
    waitForMessage(~name="Ready",
    msg => msg == Ready, context);
  }
